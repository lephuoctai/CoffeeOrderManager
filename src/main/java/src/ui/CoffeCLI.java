...
public class CoffeCLI implements Runnable{
    ...
    private static final String TOP = line('┌','┬','┬','┐');
    ...
    public void run() {
        while(true) {
            ...
            addLines(sb, new String[]{
                    "Doanh thu: " + Currency.VND.getFormat(Bank.getInstance().getBalance()),
                    "Cập nhật lúc " + Bank.getInstance().getLastUpdated().format(DateTimeFormatter.ofPattern("HH:mm dd-MM-yyyy"))
            });
            sb.append(menu());
            ...
            if (Machine.getInstance().checkout()) System.out.println(Machine.getInstance().getBill());
            ...
            if (input.equals("Y") && Machine.confirmOrder()) {
                addLines(sb, new String[]{"Thanh toán thành công! Cảm ơn bạn đã mua hàng.", "Nhấn Enter để tiếp tục..."});
            } else if (input.equals("N")) {
                addLines(sb, new String[]{"Đơn hàng đã bị hủy.", "Nhấn Enter để tiếp tục..."});
            } else {
                addLines(sb, new String[]{"Thanh toán thất bại: " , Machine.getInstance().getErrorMessage(), "Nhấn Enter để tiếp tục..."});
            }
    ...
    private void orderDrink(){
        DrinkOrder drinkOrder = Machine.getInstance().createOrder();
        Scanner scanner = new Scanner(System.in);

        System.out.println(HEAD);
        try {
            int coreId = inputId(scanner,"Nhập số thứ tự món nền: ",Machine.getCoreDrinks().size());
            int sizeId = inputId(scanner,"Nhập số thứ tự kích thước : ",Machine.getSizes().size());
            int milkId = inputId(scanner,"Nhập số thứ tự loại sữa: ",Machine.getMilkTypes().size());

            drinkOrder.add(Machine.getCoreDrinks().get(coreId - 1)).add(Machine.getSizes().get(sizeId - 1)).add(Machine.getMilkTypes().get(milkId - 1));

            System.out.println("Nhập số thứ tự topping (0 để kết thúc):");
            while (true) {
                int toppingId = inputToppingId(scanner,Machine.getToppings().size());
                if (toppingId == 0) break;
                drinkOrder.add(Machine.getToppings().get(toppingId - 1));
            }
        } catch (Exception e) {
    ...
    private String menu() {
        StringBuilder sb = new StringBuilder();
        String[] headers = {"Số", "Tên", "Giá"};

        sb.append("\n");
        addTitle(sb,"M E N U");

        addList(sb, "Món nền", Machine.getCoreDrinks(), headers);
        addList(sb, "Kích thước", Machine.getSizes(), headers);
        addList(sb, "Loại sửa", Machine.getMilkTypes(), headers);
        addList(sb, "Topping", Machine.getToppings(), headers);
